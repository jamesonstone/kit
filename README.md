...
| `git wt`, `git wt list [--root-position top\|bottom]` | Select a worktree with the primary checkout pinned and marked `[home]`; literal `main` worktrees are marked `[main]`; both use bright magenta; `PR#` is a two-second fail-soft GitHub annotation, local `LAST UPDATED` includes the day and `HH:MM`, and `h` opens home |
