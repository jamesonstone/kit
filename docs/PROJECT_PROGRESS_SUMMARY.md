...
| 0063 | explicit-work-lane-choice | `docs/specs/0063-explicit-work-lane-choice` | deliver | no | 2026-08-27 | Default every coding-agent repository mutation to a new pull-request worklane without asking, keep the primary checkout read-only, and permit existing-lane continuation only when explicitly directed. |
...
- **STATUS**: deliver
- **PAUSED**: no
- **INTENT**: Default every coding-agent repository mutation to a new pull-request worklane without asking.
- **APPROACH**: Allocate a complete new issue/branch/worktree/ready-PR lane after read-only recon, honor only explicit same-scope continuation, preserve the landing plan and primary-checkout gates, align active guidance and managed prompts, and publish additive instruction version v11 without changing v1-v10.
