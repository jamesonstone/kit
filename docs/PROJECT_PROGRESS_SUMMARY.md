...
| 0017 | reconcile-command | `docs/specs/0017-reconcile-command` | reflect | no | 2026-07-08 | - Add a `kit reconcile [feature]` command that audits Kit-managed project documents against the current Kit document contract and outputs a prompt for an agent to reconcile stale or missing documentation. - The original v1 command defaults to whole-project reconciliation and emits exact file targets, update instructions, and codebase search guidance instead of editing docs directly. - GH-52 extends project-wide `kit reconcile` into the reviewed refresh surface for Kit-managed files, rulesets, and follow-up coding-agent prompts. |
...
| 0050 | safe-worktree-workflow | `docs/specs/0050-safe-worktree-workflow` | complete | no | 2026-07-27 | Provide a portable native Git worktree policy plus an optional thin `git wt` convenience for exact navigation, bounded pull-request annotations, default writable-lane `.env` and `.envrc` symlinks, explicit isolation, and conservative removal. |
...
| 0057 | infrastructure-change-approval | `docs/specs/0057-infrastructure-change-approval` | complete | no | 2026-08-07 | Require one plan-level confirmation and one-pass execution per bounded public-cloud, Kubernetes, or infrastructure-as-code batch, consolidate follow-up changes, and always confirm deletion or removal after its summary. |
...
- **STATUS**: complete
- **PAUSED**: no
- **INTENT**: Provide a portable native Git worktree policy plus an optional thin `git wt` convenience for exact navigation, bounded pull-request annotations, default writable-lane `.env` and `.envrc` symlinks, explicit isolation, and conservative removal.
- **APPROACH**: 1. Pass a default-enabled environment-link option through the shared writable-lane path while leaving detached PR, migration, and root behavior unchanged. 2. Link exact primary-checkout `.env` and `.envrc` sources, preserve existing destination `.envrc` material, refuse unexpected links, and support `--no-link-env` isolation for both. 3. Recognize only verified managed environment symlinks during safe removal and restore all of them if native removal fails. 4. Print exact registered lane paths for safe shell navigation without fuzzy matching. 5. Cover the complete behavior with real-Git integration tests and command-help assertions. 6. Make native `git worktree` and exact filesystem operations authoritative in active rules, generated V3 guidance, and downstream managed projects while preserving immutable V1/V2 payloads. 7. Keep `git wt` documented only as an optional manual convenience that implements the portable contract without directing reconciliation. 8. Resolve environment ownership from the primary worktree and distribute the native guide as a V3 reconcile support artifact. 9. Annotate list rows with exact open-PR matches through one two-second batched `gh` lookup whose distinct failure markers never block local listing. 10. Add interactive PR titles through the same lookup, reserve complete paths before assigning the flexible title width, and preserve plain-output compatibility.
- **OPEN ITEMS**: review and merge issue #119's ready pull request #120
...
- **STATUS**: complete
- **PAUSED**: no
- **INTENT**: Require Kit-managed coding agents to outline public-cloud, Kubernetes, and infrastructure-as-code mutations in the task plan, obtain one confirmation per complete batch, execute each batch in one pass, consolidate later required changes, and always confirm deletion or removal after its summary.
