...
- **STATUS**: deliver
- **PAUSED**: no
- **INTENT**: Terminal completion tables are harder to scan than expected. Keep the status-first envelope, but put blockers and next steps in a prioritized list instead of a Markdown pipe table.
- **APPROACH**: Rewrite the canonical completion ruleset and shared instruction gate, add immutable `v8` equal to `v7` except the completion section, keep `v6`/`v7` frozen, and land the change through issue #174.
- **OPEN ITEMS**: none
