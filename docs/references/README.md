...
- Use `rules/github-pr-merge.md` and resolve `pull-request-merge` before any merge or merge-queue mutation; explicit bounded standing authority may bind later in-scope PRs and refreshed heads, but every resolved exact current node must be `MERGE_READY`
