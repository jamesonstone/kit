...
| `infrastructure-change-approval` | cloud, infrastructure-as-code, AWS, GCP, Azure, Kubernetes, Terraform, Pulumi, CloudFormation | One plan-level confirmation and one-pass execution per covered infrastructure batch, excluding routine application operations, with name-aware material AWS targets and explicit confirmation for deletion or removal. |
