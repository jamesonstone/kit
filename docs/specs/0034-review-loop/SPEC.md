...
Add a Kit-native review-loop workflow, exposed through `kit review-loop` and the `kit dispatch --loop` alias, that turns current unresolved CodeRabbit PR feedback into a human-reviewed dispatch prompt with optional review-completion waiting and no git or GitHub mutation without explicit approval.
...
- Provide a first-class Kit review-loop workflow for pull requests with CodeRabbit feedback, available through both `kit review-loop` and `kit dispatch --loop`.
...
- Expose the command behavior through `kit capabilities` once a runnable `review-loop` command exists.

## NON-GOALS

- Do not automatically stage, commit, push, merge, resolve review threads, post PR comments, or trigger GitHub mutations.
- Do not resolve review threads as part of default `kit review-loop`, `kit dispatch --loop`, or `kit dispatch --pr` prompt generation; resolution is allowed only through an explicit follow-up mutation command after fixes or no-op decisions are complete.
...
- [SPEC-01] Kit must expose a dedicated review-loop workflow surface through `kit review-loop` and a functional alias through `kit dispatch --loop`.
...
- [SPEC-26] README and agent-facing docs must describe when to use `review-loop`, `dispatch --loop`, and `dispatch --pr --coderabbit`.
...
- [ACCEPT-02] CLI help exposes `kit review-loop`, `kit dispatch --loop`, and their important flags.
...
- [ACCEPT-18] Tests prove `kit review-loop` and `kit dispatch --loop` route to equivalent review-loop behavior.
- [ACCEPT-19] `go run ./cmd/kit capabilities review-loop --json` emits a targeted record after the command is registered.
- [ACCEPT-20] `go run ./cmd/kit capabilities --search review-loop --json` identifies the relevant review-loop surface.
- [ACCEPT-21] README and agent-facing docs distinguish `kit review-loop`, `kit dispatch --loop`, and `kit dispatch --pr --coderabbit`.
