...
- [PLAN-02A][SPEC-08][SPEC-09][SPEC-10][SPEC-11] Keep `SPEC.md` reference inventories separate from `## SKILLS` and require exact locations for design dependencies.
...
- `SPEC.md` gains a required `## SKILLS` section and a reference inventory for newly generated or touched specs.
...
- Verify `kit spec` prompt output names all required discovery inputs, the `## SKILLS` table requirement, and the separate reference inventory rules.
