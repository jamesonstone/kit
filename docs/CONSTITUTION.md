...
- Before a substantial terminal completion or handoff response, load `docs/references/rules/agent-completion-output.md` and report only What happened, Deviations, and Next steps, omitting empty sections and keeping the report short enough to read in one pass; answer ordinary conversational requests naturally without that structured envelope.
