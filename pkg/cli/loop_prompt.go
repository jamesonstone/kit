...
func appendLoopContract(prompt string, stage loopStage, minConfidence int) string {
	...
	builder.WriteString("- During the clarify stage, research only discoverable facts. If user intent is still ambiguous, set `status` to `blocked`, include a clearly labeled `Open Questions` section with exact numbered questions, recommendations, assumptions, and why each answer matters; do not guess.\n")
	builder.WriteString("- If no user input remains, output `Understanding: <n>%`, `Open Questions: none`, and `No more input required from the user` before the result line.\n")
