...
func workflowCapabilityRecords() []capabilityRecord {
	return []capabilityRecord{
		...
		capability("dispatch", "Agent Workflow", "Produce an accountable Agent Team Plan prompt or PR-feedback repair prompt.", mutationGit,
			...
			withFlags(flag("--file", "read task input from a file"), flag("--pr", "ingest active PR review feedback"), flag("--loop", "use asynchronous review-loop intake"), flag("--resolve", "resolve verified handled threads", "GitHub mutation with --yes"), flag("--max-subagents", "default 3, hard ceiling 4"), flag("--output-only", "print prompt output")),
		...
		capability("instructions", "Agent Workflow", "Print immutable provider-neutral coding-agent instructions.", mutationNone,
			...
			withExamples("kit instructions", "kit instructions --version v3"),
