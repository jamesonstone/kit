...
func workflowCapabilityRecords() []capabilityRecord {
	return []capabilityRecord{
		...
		capability("instructions naming", "Agent Workflow", "Print shared conversation naming policy and host adapters.", mutationNone,
			withNetwork("none"), withFileWrites("none"), withGitMutation("none"),
			withExamples("kit instructions naming"),
			withCaveats("Policy only; no host session access or mutation.")),
		capability("instructions title", "Agent Workflow", "Format agent-resolved conversation ownership and compare its title.", mutationNone,
			withNetwork("none"), withFileWrites("none"), withGitMutation("none"),
			withFlags(flag("--scope", "owning repository or stable program scope"), flag("--domain", "stable conceptual area"), flag("--objective", "current owned outcome"), flag("--current", "current title"), flag("--accurate", "preserve current title when ownership remains accurate"), flag("--json", "emit title, changed and reason")),
			withExamples("kit instructions title --scope labcore --domain eventing --objective 'prod delivery'"),
			withCaveats("Does not infer semantics or rename a host session; unchanged normal output is empty.")),
