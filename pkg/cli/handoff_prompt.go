...
func projectHandoffWithConfig(projectRoot string, cfg *config.Config) (string, error) {
	...
	if len(activeFeatures) > 0 {
		sb.WriteString("4. For each active feature, compare current implementation reality, task state, and repository findings against the listed feature docs.\n")
		...
		sb.WriteString("6. Update `PROJECT_PROGRESS_SUMMARY.md` so it reflects the reconciled state of every active feature.\n")
		...
		sb.WriteString("5. Update any stale project docs first so the handoff is accurate.\n")
		sb.WriteString("6. Keep changes limited to documentation and handoff accuracy. Do not begin unrelated implementation work.\n")
		sb.WriteString("7. If a listed doc is stale, update it before producing your final handoff response.\n")
		sb.WriteString("8. Prefer repository files and current code over memory when they disagree.\n\n")
	}
	...
	sb.WriteString("   - one concise paragraph confirming all relevant documentation files have been updated and are up to date\n")
	sb.WriteString("   - if you updated docs, name the files you changed in that paragraph\n")
...
func featureHandoff(featureRef string) (string, error) {
	...
	sb.WriteString("4. Compare current implementation reality, task status, and repository findings against each feature document.\n")
	...
	sb.WriteString("6. Keep `PROJECT_PROGRESS_SUMMARY.md` aligned with the reconciled feature state.\n")
	sb.WriteString("7. Limit your work to documentation reconciliation and handoff preparation. Do not start unrelated implementation work.\n")
	if feat.Phase == feature.PhaseSpec || feat.Phase == feature.PhasePlan {
		sb.WriteString("8. Preserve the planning approval semantics when they still apply: " + approvalSyntaxSummary + ".\n")
	} else {
		sb.WriteString("8. Be explicit about the actual implementation and task state, especially when code has outpaced `PLAN.md` or `TASKS.md`.\n")
	}
	sb.WriteString("9. Prefer repository files and current code over memory when they disagree.\n\n")
...
func genericHandoffInstructions() string {
	...
	sb.WriteString("5. Prefer repository files and current code over memory when they disagree\n\n")
	sb.WriteString("## Final Response Contract\n\n")
...
func featureScopedDocuments(feat *feature.Feature) []handoffDocument {
	entries := []struct {
		...
		{"BRAINSTORM.md", fmt.Sprintf("Upstream research for %s; preserve validated findings and affected-file context", feat.Slug)},
		{"SPEC.md", fmt.Sprintf("Feature requirements for %s; keep scope, acceptance, and edge cases aligned with reality", feat.Slug)},
		{"PLAN.md", fmt.Sprintf("Implementation approach for %s; update when execution diverges from the written design", feat.Slug)},
