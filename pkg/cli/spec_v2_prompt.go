...
func buildSpecV2SupervisorPrompt(input specV2PromptInput) string {
	...
	return renderPromptDocument(func(doc *promptdoc.Document) {
		...
		doc.BulletList(
			...
			"Clarification Mode output is a pre-instruction report plus a clearly labeled `Open Questions` section.",
			"`Open Questions` must be maximized for user review: use numbered questions, readable spacing, recommended defaults, assumptions, uncertainty resolved, and exactly what answer format the user can use.",
			"If open questions remain, each question must include `Recommendation`, `Assumption`, and `Why this matters` lines so the user can review and approve quickly.",
			"If no open questions remain, output `Understanding: <n>%`, `Open Questions: none`, and `No more input required from the user` before continuing.",
		...
		doc.BulletList(
			...
			"Use a visibly labeled `Open Questions` section for every clarification batch.",
			...
			"When no more questions remain, state the current understanding percentage, state `Open Questions: none`, and state that no other input is required from the user.",
		...
		doc.BulletList(
			"Clarification-loop replies must include a clearly labeled `Open Questions` section with numbered questions, recommendations, assumptions, uncertainty, confidence, and unresolved count; do not append the full final-response contract to each clarification batch.",
			"If no questions remain, write `Understanding: <n>%`, `Open Questions: none`, and `No more input required from the user` before continuing or summarizing readiness.",
