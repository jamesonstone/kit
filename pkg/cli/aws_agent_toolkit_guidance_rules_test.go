...
func TestAWSAgentToolkitGuidanceRulesetIsValid(t *testing.T) {
	...
	for _, check := range []string{
		...
		"reference/ec2/describe-regions.html",
		"reference/account/get-account-information.html",
		"parameter-store-public-parameters-global-infrastructure.html",
		...
		"default `region`",
		"query `aws ec2 describe-regions` for Regions enabled for that account",
		"Do not maintain a static Region catalog",
		"run `kit aws verify` before the first AWS-dependent command",
		"verified account ID, ARN, and Region as authoritative",
		"large or materially risky",
