...
func TestGitHubPRDeliveryRulesetUsesAutonomousRecovery(t *testing.T) {
	...
	for _, check := range []string{
		...
		"capture the exact version-control-eligible paths it owns and each path's pre-command state",
		"abort rather than overwrite or combine ambiguous state",
		"removed paths must remain absent",
		"the staged index must contain exactly the captured paths",
		"restore the protected root checkout's captured paths to their exact pre-command states",
