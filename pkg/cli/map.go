...
func runMap(cmd *cobra.Command, args []string) error {
	...
	if len(args) == 0 {
		if mapContext {
			return fmt.Errorf("--context requires a feature argument")
		}
		if mapJSON {
	...
	for _, featureMap := range projectMap.Features {
		if featureMap.Feature.DirName == feat.DirName {
			if mapContext {
				if mapJSON {
					return outputContextPlanJSON(cmd.OutOrStdout(), featureMap)
				}
				return outputFeatureContextPlan(cmd.OutOrStdout(), featureMap)
			}
			if mapJSON {
				return outputMapJSON(cmd.OutOrStdout(), featureMap)
			}
