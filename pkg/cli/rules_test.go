...
func TestGitHubPRDeliveryRulesetUsesAutonomousRecovery(t *testing.T) {
	...
	for _, check := range []string{
		...
		"version-control-eligible unstaged or untracked files created or updated by that command",
		"Verify the destination content and complete diff before removing only the transferred source state",
		"stage their exact paths",
