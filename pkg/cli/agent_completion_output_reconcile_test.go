...
func TestReconcileFindsStaleAgentCompletionOutputGuidance(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			path:    "AGENTS.md",
			snippet: "Write every response, including terminal completions and handoffs, in the shape the content calls for",
		...
		},
		{
			...
			version: config.InstructionScaffoldVersionMemory,
			path:    "AGENTS.md",
			snippet: "never leave the reader wrong about a blocker",
			audit:   auditV3SupportGuidance,
		},
		{
		...
		},
		{
			...
			version: config.InstructionScaffoldVersionMemory,
			path:    "AGENTS.md",
			snippet: "Do not emit the retired envelope",
			audit:   auditV3SupportGuidance,
		},
		{
			...
			version: config.InstructionScaffoldVersionMemory,
			path:    "AGENTS.md",
			snippet: "There is no required response format",
			audit:   auditV3SupportGuidance,
		},
		{
			...
			snippet: "They name facts that must reach the reader, never a layout",
			audit:   auditV3SupportGuidance,
		},
		{
