...
func capabilityCatalog() []capabilityRecord {
	records := []capabilityRecord{
		...
		capability("loop", "Workflow", "Run or prepare iterative agent workflow loops.", mutationExecutesCommands, withFileWrites("can write loop state under .kit/loops"), withGitMutation("none", "delegated agent commands may mutate git independently"), withFlags(flag("--dry-run", "show loop plan without delegated execution", "read-only"), flag("--max-iterations", "bound loop execution")), withRelated(related("implement", "runs outside the loop runner"))),
		...
		capability("capabilities", "Inspect & Repair", "List Kit commands and their safety behavior.", mutationNone, withFlags(flag("--json", "emit stable machine-readable capability data"), flag("--full", "include detailed hidden and deprecated records"), flag("--search", "filter visible command metadata")), withRelated(related("map", "shows feature context"), related("help", "shows command syntax")), withWhenToUse("Use before choosing a Kit command when command behavior, mutation, network, or file-write boundaries are uncertain.", "Use targeted lookup before implementing or extending a Kit command so the command catalog stays current."), withWhenNotToUse("Do not use as a replacement for `kit map` when the question is about feature documents or references.", "Do not use `--full` by default; prefer compact search or targeted lookup unless hidden/deprecated command metadata is needed."), withExamples("kit capabilities --json", "kit capabilities dispatch --json", "kit capabilities --search review-loop --json"), withCaveats("Every command-surface change must update this catalog; see docs/references/rules/command-capabilities.md.")),
		...
		capability("review-loop", "Prompt Utilities", "Prepare a dispatch prompt from current unresolved PR review feedback without mutating git or GitHub.", mutationNone, withNetwork("uses gh to read PR metadata, review threads, and optional CodeRabbit check status"), withFileWrites("none by default", "editor and clipboard output use temporary/editor state outside project files"), withGitMutation("none"), withFlags(flag("--pr", "select the pull request from a URL, Markdown link, owner/repo#number, or current-repo number", "network read"), flag("--coderabbit", "include only CodeRabbit-authored review comments and Prompt for AI Agents blocks"), flag("--watch", "wait for current-head CodeRabbit review completion before collecting feedback", "network read with polling"), flag("--copy", "copy generated dispatch prompt output"), flag("--output-only", "print prompt without wrapper text"), flag("--max-subagents", "bound the generated dispatch queue")), withRelated(related("dispatch --loop", "alias path for the same review-loop workflow"), related("dispatch --pr --coderabbit", "raw PR review-thread intake without triage or watch mode"), related("ci", "inspects GitHub Actions failures")), withExamples("kit review-loop --pr 14 --coderabbit", "kit review-loop --pr owner/repo#14 --watch"), withCaveats("This command intentionally does not resolve review threads; use `kit dispatch --pr <target> --resolve --yes` only after fixes or no-op decisions are complete.")),
		capability("dispatch", "Prompt Utilities", "Build dispatch prompts for agents, PR review threads, and CodeRabbit review-loop intake.", mutationNetwork, withNetwork("none by default", "--pr fetches unresolved, non-outdated GitHub PR review threads through gh api graphql; --loop also reads PR metadata and optional CodeRabbit check status; --resolve --yes mutates GitHub review-thread state"), withFileWrites("none by default", "editor, --copy, and output flags can copy or write generated prompt output outside project files"), withGitMutation("none"), withFlags(flag("--pr", "prefill the dispatch editor from unresolved, non-outdated PR review threads", "network read"), flag("--loop", "route PR review feedback through the review-loop workflow"), flag("--coderabbit", "with --pr, keep CodeRabbit-authored comments and extract Prompt for AI Agents blocks"), flag("--resolve", "with --pr, resolve matching unresolved review threads after fixes or no-op decisions are complete", "GitHub mutation; requires --yes"), flag("--yes", "confirm --resolve after fixes or no-op decisions are complete", "required for mutation"), flag("--watch", "with --loop, wait for current-head CodeRabbit review completion", "network read with polling"), flag("--copy", "copy generated prompt output"), flag("--output-only", "print prompt without wrapper text"), flag("--max-subagents", "bound the generated dispatch queue")), withRelated(related("review-loop", "canonical review-loop command"), related("ci", "inspects CI context"), related("code-review", "builds review-focused prompts")), withExamples("kit dispatch --file tasks.md", "kit dispatch --pr 14 --coderabbit", "kit dispatch --loop --pr 14 --watch", "kit dispatch --pr 14 --resolve --yes"), withCaveats("Default dispatch generates prompts only; `--resolve --yes` is the explicit GitHub mutation path for already-handled review threads.")),
