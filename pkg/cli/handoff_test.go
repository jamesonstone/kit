...
func TestProjectHandoffIncludesProgressSummaryAndStatus(t *testing.T) {
	...
	checks := []string{
		...
		"SPEC.md phase/task/evidence state",
		"canonical front matter phase, references, relationships, skills, and evidence remain current",
...
func TestFeatureHandoffIncludesDocSyncInstructions(t *testing.T) {
	...
	checks := []string{
		...
		filepath.Join(projectRoot, "docs", "specs", "0001-alpha", "PLAN.md"),
		filepath.Join(projectRoot, "docs", "specs", "0001-alpha", "TASKS.md"),
		"Compare current implementation reality, SPEC.md phase/task/evidence state",
		"If any feature specification document is stale, update it first",
		"Verify that `SPEC.md` keeps canonical front matter phase, references, relationships, skills, validation evidence, and delivery state current",
		"all relevant documentation files and reference inventories have been updated and are up to date",
		"## Final Response Contract",
		"`Documentation Sync`",
		"`Documentation Files`",
		"`Recent Context`",
