...
func TestProjectHandoffIncludesProgressSummaryAndStatus(t *testing.T) {
	...
	checks := []string{
		...
		"phase reference inventories",
		"canonical front matter references list current `active`, `optional`, and `stale` references with exact targets",
...
func TestFeatureHandoffIncludesDocSyncInstructions(t *testing.T) {
	...
	checks := []string{
		...
		"Verify that `BRAINSTORM.md`, `SPEC.md`, and `PLAN.md` keep canonical front matter references current",
