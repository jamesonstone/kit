...
func managedFileDeliveryInstructionsForCommand(
	...
	instructions := []string{
		...
		"Before any repository or delivery mutation, default to a new worklane without asking unless the user explicitly directed continuation of an existing lane for this scope; record the repository, issue, branch, non-primary worktree, protected base, and create-or-update pull-request target in a Pull-Request Landing Plan.",
		"Treat exact existing-PR review repair, CI repair, base refresh, and ordered merge coordination as continuation: reuse every targeted head branch and pull request, and never create a coordinator or corrective pull request for scope-preserving work.",
		"If the snapshot came from the primary checkout or was created before that lane and plan were established, trigger the work-lane tripwire: preserve it and do not adopt, transfer, stage, commit, push, restore, discard, stash, reset, or clean it until the worktree pull request has been merged. After that merge, follow the post-merge primary leftover cleanup below.",
...
func managedFileDeliveryInstructionsWithoutSnapshot(projectRoot string, rerunCommands ...string) []string {
	...
	instructions := []string{
		...
		"Before any repository or delivery mutation, default to a new worklane without asking unless the user explicitly directed continuation of an existing lane for this scope; record the complete Pull-Request Landing Plan.",
		"Treat exact existing-PR review repair, CI repair, base refresh, and ordered merge coordination as continuation: reuse every targeted head branch and pull request, and never create a coordinator or corrective pull request for scope-preserving work.",
		"For the default new lane, create or reuse the human-assigned GitHub issue, exact `GH-<issue-number>` branch, and canonical non-primary writable worktree; for an explicitly continued existing lane, prove its exact branch, owning non-primary worktree, issue scope, and create-or-update pull-request target.",
...
func managedFileDeliveryClosingInstructions() []string {
	return []string{
		...
		"Merge the worktree pull request only when active standing authority covers its resolved exact current node and it is `MERGE_READY`, following `github-pr-merge`. A later in-scope head does not require renewed authority. This leftover cleanup does not invent readiness.",
