...
func addContextContract(document *promptdoc.Document, config Config) {
	...
	document.Paragraph("An accepted task or active `/goal` authorizes in-scope non-destructive merges once they are `MERGE_READY`. PR-delivery consent, successful checks, subagent assignment, and a program ledger do not invent readiness. A changed head loses readiness, not accepted-task authority. Stop for exact confirmation only for destructive effects or an explicit user hold; clarify material product-scope expansion.")
