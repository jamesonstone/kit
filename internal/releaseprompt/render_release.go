...
→ advance only now-ready dependent nodes`)
	...
	document.Paragraph("A commit SHA or head OID identifies readiness evidence only and never becomes an authorization identity. Do not ask for renewed or exact-head authorization solely because a later in-scope PR or refreshed head changed identifiers. After final-head evidence restores `MERGE_READY`, continue the authorized standard deployment and browser retry only while all current authorization, identity, policy, workflow, environment, dependency, and material-effect gates remain valid.")
