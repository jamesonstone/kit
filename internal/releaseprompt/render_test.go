...
func TestRenderIsDeterministicCompleteAndAuthorityAware(t *testing.T) {
	...
	for _, required := range []string{
		...
		"pull-request-merge",
		"Standing merge or deployment authority exists only when a human explicitly authorizes a bounded task, goal, or program",
		"Standing authority may bind later-created in-scope PRs and refreshed heads",
		"SHA/head OID is evidence only, never an authorization identity",
		"Do not request exact-head reauthorization",
		"continue the authorized standard deployment and browser retry",
		"matching `MERGE_READY` frontier",
		"Pending, missing, stale-head",
		"Report partial waves literally",
		"Destructive Or Data-Loss Changes",
...
func goldenConfig() Config {
	return Config{
		...
		Infrastructure: Infrastructure{
			...
			Policy:        "Covered infrastructure changes require explicit bounded approval; standard artifact deployment uses standing authority.",
		},
