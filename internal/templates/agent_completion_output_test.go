...
func TestInstructionTemplatesRequireAgentCompletionOutput(t *testing.T) {
	required := []string{
		...
		"State plainly whether work is finished, partly finished, blocked, or failed",
		"no announcement that there are no deviations, risks, or next steps",
		"match length to consequence rather than to effort spent",
		...
		"Omit a commit SHA cited as proof that something was checked",
		"They name facts that must reach the reader, never a layout",
		"Include an identifier only when the reader needs it to act",
		"preserve literal states such as PENDING, UNKNOWN, SKIPPED, and NOT_APPLICABLE",
	}
