...
func TestInstructionTemplatesRequireAgentCompletionOutput(t *testing.T) {
	required := []string{
		...
		"target twelve rendered lines or fewer, at most five What happened bullets",
		"omit Deviations and Next steps entirely when empty instead of writing a None bullet",
		"on its own line, then one to three plain sentences of prose",
		"reserve bold for the status line, blockers, and required actions",
	...
	for name, content := range map[string]string{
		...
		for _, forbidden := range []string{
			...
			"in the first What happened bullet",
