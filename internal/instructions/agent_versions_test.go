...
func TestAgentInstructionsRejectsUnavailableVersion(t *testing.T) {
	_, err := AgentInstructions("v14")
	if err == nil {
		t.Fatal("AgentInstructions(\"v14\") expected an error")
	}
	for _, want := range []string{`unsupported instructions version "v14"`, "available versions: v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13"} {
		if !strings.Contains(err.Error(), want) {
			t.Fatalf("AgentInstructions(\"v14\") error = %q, want %q", err, want)
		}
	}
}
...
func TestAgentInstructionVersionsReturnsCopy(t *testing.T) {
	...
	if len(versions) != 13 || versions[0] != "v1" || versions[1] != "v2" || versions[2] != "v3" || versions[3] != "v4" || versions[4] != "v5" || versions[5] != "v6" || versions[6] != "v7" || versions[7] != "v8" || versions[8] != "v9" || versions[9] != "v10" || versions[10] != "v11" || versions[11] != "v12" || versions[12] != "v13" {
		t.Fatalf("AgentInstructionVersions() = %v, want [v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 v11 v12 v13]", versions)
	}
